proc cas;
    source ds;
        data &outputCaslib..loans_raw(drop=Date initialCurrentCategories CurrentCategories ) /*accounts*/
             &outputCaslib..customers(keep=CurrentCategories ID Salary Date Age EmpLength); /*customer info */
            call streaminit(99);
...
* Save the loans_raw in-memory table as a data source file *;
	table.save / 
		table={name='loans_raw', caslib="&outputCaslib"},
		name='loans_raw.sashdat', caslib="&outputCaslib",
		replace=TRUE;
...
* Drop the in-memory table *;
	table.dropTable / name='loans_raw', caslib="&outputCaslib", quiet=TRUE;
quit;