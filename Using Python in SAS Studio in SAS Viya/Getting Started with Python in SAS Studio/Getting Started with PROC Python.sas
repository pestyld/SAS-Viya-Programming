...
SAS.df2sd(df, 'work.hmeq_compute_sas')
...
proc casutil;
	* Drop the global scope CAS table if it exists *;
	droptable casdata='hmeq_cas_sas' incaslib="casuser" quiet;

	* Send the SAS data set to the CAS server and promote the table *;
	load data=work.hmeq_compute_sas casout="hmeq_cas_sas" outcaslib="casuser" promote;
