...
SAS.df2sd(df, 'work.hmeq_compute_py')


## Create a string with SAS code to connect to CAS server, delete the global table if it exists, and then load data to CAS
## NOTE: You can also use the Python SWAT package to accomplish the same tasks below.
...
proc casutil;
	* Drop the global scope CAS table if it exists *;
	droptable casdata='hmeq_cas_py' incaslib="casuser" quiet;

	* Send the SAS data set to the CAS server and promote the table *;
	load data=work.hmeq_compute_py casout="hmeq_cas_py" outcaslib="casuser" promote;
